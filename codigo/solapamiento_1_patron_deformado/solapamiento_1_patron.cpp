/*
	...
	La finalidad de este programa es determinar la convergencia del coeficiente de solapamiento y la energia a disitintas tmperaturas introduciendo una 
	condicion inicial deformada
	...
	if(H.Entrenar("inicio/modelo.dat",1) && z.Leer("inicio/modelo.dat")){
		...
		if(E_T.is_open() && M_T.is_open()){
			...
			do{
                ...
                desviacion("caracteristicas/evolucion/"+aux+".dat",3,3,vector);
                
				//Guardamos la energia
				E_T<<T<<" "<<vector[0]<<" "<<abs(vector[0])<<" "<<vector[1]<<endl;
                
                desviacion("caracteristicas/evolucion/"+aux+".dat",2,3,vector);
