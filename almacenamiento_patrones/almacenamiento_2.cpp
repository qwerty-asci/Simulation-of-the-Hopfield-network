/*
	...
	Programa para determinar la evolucion del coeficiente de solapamiento en funcion del numero de patrones almacenados
...
int main(){
	...
	double fb=0.0,rand=0.0,promedio_a[N_max_patrones],aux,a,vect[2],a_tabla[N_max_patrones][N_rep][4],res[4];
	...
	for(unsigned long long int q=0;q<N_rep;q++){
		...
		if(Dat.is_open()){
			...
			for(unsigned long long int k=N_min;k<=N_max_patrones;k++){
	...
	if(Dat.is_open()){
		...
		for(unsigned long long int i=N_min;i<N_max_patrones;i++){
