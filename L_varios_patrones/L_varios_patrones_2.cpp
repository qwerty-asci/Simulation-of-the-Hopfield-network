...
int main(){
    ...
    double N,paso,pasos,T_max,T_0,T_min,hT,crite_estb;
    
    long long unsigned int fact,N_max_patrones;
    ...
    cout<<"Numero maximo de patrones a almacenar: "<<endl;
    cin>>N_max_patrones;
	...
	double rand=0.0,E=0.0,T=T_0,vector[2],p2;
		...
		if(H->Entrenar("inicio/modelo.dat",k)){
			...
			if(E_T.is_open() && M_T.is_open()){
				...
				do{
                    ...
                    desviacion("caracteristicas/"+to_string(k)+"_patron/evolucion/"+aux+".dat",k+2,k+2,vector,pasos,fact,crite_estb);
					...
					for(unsigned int j=1;j<=k;j++){
                        desviacion("caracteristicas/"+to_string(k)+"_patron/evolucion/"+aux+".dat",j+1,k+2,vector,pasos,fact,crite_estb);
